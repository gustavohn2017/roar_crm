...
def admin_required(view_func):
    """Restringe o acesso apenas para usuários com papel de administrador."""
    return login_required(user_passes_test(lambda u: hasattr(u, 'profile') and u.profile.role == 'admin', login_url='/login/')(view_func))

def supervisor_or_admin_required(view_func):
    """Restringe o acesso para usuários supervisores ou administradores."""
    @login_required
    @wraps(view_func)
    def _wrapped_view(request, *args, **kwargs):        # Verifica se o usuário tem o atributo profile
